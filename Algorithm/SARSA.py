...
class SARSA:
        ...
        for learner in self.learners:
            ...
            for i in range(self.grid_world.get_width()):
                for j in range(self.grid_world.get_height()):
                    Q_tables[learner.get_id()][(i, j)] = {'UP': 1, 'DOWN': 1, 'RIGHT': 1, 'LEFT': 1}
        return Q_tables
    ...
    def update_Q_value(self, learner, state1, action1, state2, action2, reward):
        """
        SARSA update rule :
        ...
        next_q_value = Q_table[state2][action2]     # max_a Q(s',a)
        ...
        for episode in range(self.total_episodes):
            ...
            for t in range(self.episode_length):
                ...
                for learner in self.learners:
                    ...
                    print('next state : ', state2)
                    action2 = self.choose_action(learner, state2)
                    print('next action : ', action2)
                    self.update_Q_value(learner, state1, action1, state2, action2, reward)
